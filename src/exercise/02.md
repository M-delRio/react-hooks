...
lazy state initialization: pass a function to .useState and the function will only be called if there is no state available, so ideally only called during 1st render vs every subsequent render following state change

Unlike componentDidMount or componentDidUpdate, effects scheduled with useEffect don’t block the browser from updating the screen. This makes your app feel more responsive. The majority of effects don’t need to happen synchronously. In the uncommon cases where they do (such as measuring the layout), there is a separate useLayoutEffect

LocalStorage only stores strings (.stringify/.parse to set/get)

Dependency array only uses shallow equality, keep in mind for objects. 
